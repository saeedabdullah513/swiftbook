...
function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* Noise / scanline */}
      <div className="absolute inset-x-0 top-0 h-16 pointer-events-none animate-scan bg-gradient-to-b from-transparent via-sunshine/20 to-transparent blur-md" />
      ...
      <div className="absolute top-24 left-1/2 -translate-x-1/2 z-10 hidden md:flex items-center gap-3 rounded-full border border-white/15 bg-white/5 backdrop-blur-xl px-4 py-1.5 text-[11px] tracking-widest uppercase">
        <span className="relative flex h-2 w-2">
          <span className="absolute inline-flex h-full w-full rounded-full bg-sunshine opacity-75 animate-ping" />
          <span className="relative inline-flex h-2 w-2 rounded-full bg-sunshine" />
        </span>
      ...
      <div className="container-editorial relative z-10 grid gap-14 lg:grid-cols-[1.15fr_1fr] items-center pt-36 pb-28 lg:pt-40 lg:pb-36">
        <div style={layer(6)} className="animate-fade-up">
          <div className="inline-flex items-center gap-2 rounded-full border border-sunshine/30 bg-sunshine/10 backdrop-blur px-4 py-1.5">
            <Sparkles className="h-3.5 w-3.5 text-sunshine" />
            <span className="text-[11px] font-semibold tracking-[0.22em] uppercase text-sunshine">
          ...
          <p className="mt-6 max-w-xl text-lg text-parchment/75 leading-relaxed">
            ...
            built from <em className="text-sunshine not-italic font-medium">your</em> book, audience, and goals.
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        {sent ? (
          <div className="mt-6 rounded-xl bg-sunshine/30 border border-sunshine p-5">
          ...
          <div className="mt-5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <SelectField
                label="Book stage"
                options={["Manuscript", "Pre-launch", "Just launched", "Backlist / Relaunch"]}
              />
            </div>
...
function Field({
  ...
  return (
    <label className="block">
      <span className="block text-[11px] font-medium tracking-wide uppercase text-ink/60">
        {label}
      </span>
      <input
        type={type}
        ...
        <div className="relative aspect-[4/5] lg:aspect-[3/4] rounded-3xl overflow-hidden shadow-lift ring-1 ring-white gradient-cool">
          {playing ? (
            ...
            <button
              ...
              className="group absolute inset-0 flex items-end p-8 text-left"
            >
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_30%,rgba(255,255,255,0.25),transparent_60%)]" />
              <div className="absolute top-6 left-6 text-white/80 text-[11px] uppercase tracking-[0.3em]">
                Play film · 2 min
              </div>
...
const BOOKS = [
  {
    ...
    results: ["68% lift in Amazon page views", "Featured in Inc. & Fast Company", "4,200 new email subscribers in 60 days"],
  },
  {
    ...
    results: ["12 podcast guest spots booked", "27k combined social views on launch", "National print syndication via AP"],
  },
  {
    ...
    results: ["Google page 1 ranking for 14 keywords", "3 national TV appearances", "Harvard Business Review contributor"],
  },
  {
    title: "Echo Street",
    author: "Nina Morris",
    ...
    results: ["#3 in Literary Fiction on launch day", "800+ pre-orders before release", "BookBub Featured Deal — 11k downloads"],
  },
  {
    ...
    results: ["33% month-over-month traffic growth", "Adopted by 6 university syllabi", "Sold foreign rights in 4 territories"],
  },
  {
    ...
    results: ["Op-ed placed in The Wall Street Journal", "2.3x increase in speaking inquiries", "First print run sold out in 3 weeks"],
  },
  {
    title: "Hidden Summer",
    author: "J. Porter",
    ...
    results: ["Top 50 in Contemporary Fiction", "300+ verified Amazon reviews", "NetGalley Most Anticipated badge"],
  },
  {
    ...
    results: ["Blog traffic grew 3x in 90 days", "Quoted in Forbes, Fortune, Bloomberg", "Keynote booking rate doubled"],
  },
  {
    title: "The Calm Edge",
    author: "Daniel Cole",
    ...
    results: ["Email list grew from 800 to 7,500", "Amazon category bestseller 14 days", "Licensed to a major wellness app"],
  },
  {
    ...
    results: ["NPR Morning Edition segment", "Paperback edition greenlit at 30k", "Publisher's Weekly starred review"],
  },
];
      ...
      <div className="mt-14 grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5">
        {BOOKS.map((b, i) => {
          ...
          return (
            <button
              key={b.title}
              onClick={() => { setActive(b); setActiveIndex(i); }}
              className="group text-left cursor-pointer border-0 bg-none p-0"
            >
              <div
...
function BookModal({ book, bookIndex, onClose }: { book: typeof BOOKS[0]; bookIndex: number; onClose: () => void }) {
  ...
  return (
    <div
      onClick={onClose}
      className="fixed inset-0 z-[100] bg-navy/70 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-up"
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="relative w-full max-w-4xl bg-white rounded-2xl shadow-lift overflow-hidden grid md:grid-cols-[1fr_1fr]"
      >
        <button
          onClick={onClose}
          className="absolute top-3 right-3 z-10 h-9 w-9 rounded-full bg-white/90 border border-warm-gray flex items-center justify-center text-navy hover:bg-white"
          aria-label="Close"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        {/* Cover side */}
        <div
          className="aspect-[3/4] md:aspect-auto relative p-6 text-white flex flex-col justify-between"
        ...
        <div className="p-6 lg:p-8 max-h-[85vh] overflow-y-auto flex flex-col gap-5">
          <video
            controls
            poster="https://images.pexels.com/photos/1181690/pexels-photo-1181690.jpeg?auto=compress&cs=tinysrgb&w=1200"
        ...
        <div className="mt-12 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {REELS.map((r, i) => (
            <button
              key={r.author}
              onClick={() => setOpen(true)}
              className="group relative aspect-[9/16] rounded-2xl overflow-hidden shadow-editorial ring-1 ring-black/5"
              style={{ background: reelBg(r.tone) }}
            >
              <div className="absolute inset-0 flex items-end p-4">
                <div className="text-white">
                  ...
                  <div className="mt-1 font-serif italic text-sm leading-snug line-clamp-3">
                    "{r.quote}"
                  </div>
                </div>
              </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex items-stretch justify-center">
      <button
        ...
        className="absolute top-4 right-4 z-20 h-11 w-11 rounded-full bg-white/15 backdrop-blur border border-white/30 text-white flex items-center justify-center hover:bg-white/25"
      >
        <X className="h-5 w-5" />
      </button>
      <div className="scroll-snap-y overflow-y-scroll h-full w-full max-w-[440px] mx-auto">
        {REELS.map((r, i) => (
          <div
            key={r.author}
            className="snap-item relative h-screen w-full flex items-end justify-center"
            style={{ background: reelBg(r.tone) }}
          >
            ...
            <div className="relative w-full p-6 pb-16 text-white">
              <div className="text-[11px] uppercase tracking-[0.3em] text-white/70">
                Reel {String(i + 1).padStart(2, "0")} of {REELS.length}
              </div>
              <div className="mt-3 font-serif italic text-3xl leading-tight max-w-sm">
                "{r.quote}"
              </div>
              ...
              <div className="mt-6 text-[11px] text-white/70">
                Scroll up for next reel · Press Esc or × to exit
              </div>
            </div>
          </div>
        ))}
...
function Platforms() {
  const platforms = [
    ...
    "Goodreads", "IngramSpark", "Draft2Digital", "BookBub", "NetGalley",
    "Audible", "Spotify",
  ];
  return (
      ...
      <div className="container-editorial">
        <div className="text-center">
          ...
          <h3 className="mt-3 font-serif text-2xl text-navy">
            Every major retailer, discovery platform, and press channel.
          </h3>
        </div>
        <div className="mt-10 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
          <div className="flex gap-3 animate-marquee w-max">
            {[...platforms, ...platforms].map((p, i) => (
              <div
                key={i}
                className="shrink-0 px-6 py-4 rounded-xl bg-parchment border border-warm-gray text-navy font-serif text-lg italic"
              >
                {p}
              </div>
            ))}
...
/* ============ FAQ + SOCIAL — Accordion with video + transcript ============ */
const FAQS = [
  {
    q: "How do you decide which services a book needs?",
    a: "We begin by reviewing the book, the audience, the current positioning, and the commercial objective. The recommended service mix comes from that assessment rather than a fixed package.",
    video: "https://www.w3schools.com/html/mov_bbb.mp4",
    poster: "https://images.pexels.com/photos/3184292/pexels-photo-3184292.jpeg?auto=compress&cs=tinysrgb&w=1200",
  },
  {
    q: "Do you guarantee bestseller results?",
    a: "No. We do not guarantee bestseller outcomes. We do guarantee strategic planning, scoped deliverables, execution, and transparent reporting with context.",
    video: "https://www.w3schools.com/html/movie.mp4",
    poster: "https://images.pexels.com/photos/3184465/pexels-photo-3184465.jpeg?auto=compress&cs=tinysrgb&w=1200",
  },
  {
    q: "Can I see what will be delivered before I commit?",
    a: "Yes. We present sample deliverables, scoped activities, timelines, and exclusions before any payment is requested — so you know exactly what you're getting.",
    video: "https://www.w3schools.com/html/mov_bbb.mp4",
    poster: "https://images.pexels.com/photos/3183153/pexels-photo-3183153.jpeg?auto=compress&cs=tinysrgb&w=1200",
  },
];
            ...
            <div className="space-y-3.5">
              {FAQS.map((f, i) => {
                ...
                return (
                  <article
                    key={f.q}
                    className="rounded-[22px] bg-white border border-warm-gray overflow-hidden shadow-[0_14px_30px_rgba(15,35,72,0.05)] transition"
                  >
                    <button
                        ...
                        <div className="px-[22px] pb-[22px]">
                          <div className="grid md:grid-cols-2 gap-4">
                            ...
                            <video
                              controls
                              poster={f.poster}
                              className="w-full h-60 object-cover rounded-2xl bg-black"
                              preload="metadata"
                            >
            ...
            <div className="rounded-3xl p-[22px] border border-warm-gray shadow-editorial sticky top-28"
              ...
              <div className="space-y-3.5">
                {SOCIAL_ITEMS.map((s) => (
                  <article
                    key={s.platform}
                    className="p-4 rounded-2xl border border-warm-gray bg-white transition hover:shadow-md"
                  >
